...
class ViewerState {
  ...
  /// Character offsets of each search match in the original [fileContent].
  final List<int> searchMatchOffsets;
  ...
  ViewerState copyWith({
    ...
    return ViewerState(
      ...
      searchMatchOffsets: searchMatchOffsets ?? this.searchMatchOffsets,
    );
  }
}
...
class ViewerNotifier extends AsyncNotifier<ViewerState> {
  ...
  Future<void> completeLoad(
    ...
    final effectiveUri = fileUri ?? file.identifier;
  ...
  /// Updates state after a "Save As" operation with new location and content
  Future<void> updateSavedAs({
    ...
    final bytes = Uint8List.fromList(utf8.encode(newContent));
    final processed = await _process(newFileName, bytes);
    ...
    if ((newFilePath != null && newFilePath.isNotEmpty) ||
        (newFileUri != null && newFileUri.isNotEmpty)) {
      unawaited(
        ref.read(historyProvider.notifier).recordFileOpen(
  ...
  void nextMatch() {
    ...
    state = AsyncData(current.copyWith(searchMatchIndex: next));
  }
  ...
  void previousMatch() {
    ...
    state = AsyncData(current.copyWith(searchMatchIndex: prev));
  }
